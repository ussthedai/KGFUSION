...
public class ConditionClusterController {
    ...
    public ConditionClusterController() throws IOException {
    ...
    public List<Map<List<String>, List<EntityEasyForNodesCluster>>> makeClusters(List<EntityEasyForNodesCluster> ens,
        ...
        // 组装聚类结果
        List<Map<List<String>, List<EntityEasyForNodesCluster>>> res = new ArrayList<>();
    ...
    @PostMapping("/conditioncluster")
    public String evolution_auto(@RequestBody String jsonStr) throws IOException{
        // input: graphSymbol, ontologySymbol, entityType, maxCategory,
        // subOntologySymbol, entitySubClass
        // 前4个字段必传且不能为空，后两个字段选择性传送
        ...
        if (!jsonObject.containsKey("graphSymbol") || !jsonObject.containsKey("maxCategory")
                || !jsonObject.containsKey("ontologySymbol") || !jsonObject.containsKey("entityType")) {
            return JSON.toJSONString(
                    new Res(false, 1002, "参数缺失,graphSymbol,ontologySymbol,entityType,maxCategory为必传参数", null));
        }

        String graphSymbol = jsonObject.getString("graphSymbol");
        String ontologySymbol = jsonObject.getString("ontologySymbol");
        String entityType = jsonObject.getString("entityType");
        int maxCategory = jsonObject.getInteger("maxCategory");


        if (jsonObject.containsKey("subOntologySymbol")) {
            subOntologySymbol = jsonObject.getString("subOntologySymbol");
        }
        if (jsonObject.containsKey("entitySubClass")) {
            entitySubClass = jsonObject.getString("entitySubClass");
        }
        ...
        if (ontologySymbol == null || ontologySymbol.equals("")) {
            return JSON.toJSONString(new Res(false, 1004, "ontologySymbol不能为空", null));
        }
        if (entityType == null || entityType.equals("")) {
            return JSON.toJSONString(new Res(false, 1005, "entityType不能为空", null));
        }
        if (maxCategory <= 0) {
            return JSON.toJSONString(new Res(false, 1006, "maxCategory必须大于0", null));
        }
        ...
        List<EntityEasyForNodesCluster> entities = new ArrayList<>();
        Map<String, String> conditions = new HashMap<>();
        conditions.put("graphSymbol", graphSymbol);
        conditions.put("ontologySymbol", ontologySymbol);
        conditions.put("entityType", entityType);
        if (subOntologySymbol != null && !subOntologySymbol.equals("")) {
            conditions.put("subOntologySymbol", subOntologySymbol);
        }
        if (entitySubClass != null && !entitySubClass.equals("")) {
            conditions.put("subEntityClass", entitySubClass);
        }
        entities = BasicOperation.queryEntityEasyForNodesClusterByConditions(conditions);
        if (entities == null || entities.size() == 0) {
            return JSON.toJSONString(new Res(false, 1008, "查询条件未返回实体,无法聚类,请调整聚类查询条件", null));
        }
        ...
        for (Entry<String, List<Map<List<String>, List<EntityEasyForNodesCluster>>>> entry : res.entrySet()) { // 根据typidx的聚类结果
            ...
            String ent = main_type.split("-")[0];
            String ensubt = main_type.split("-")[1];
            ...
            for (Map<List<String>, List<EntityEasyForNodesCluster>> bigcluster : entry.getValue()) { // 每个小的cluster，包括sourceIds,summary...
                ...
                for (String newNode : recommend_nodes) {
                    ...
                    n.setOntologySymbol(onsb);
                    n.setSubOntologySymbol(subonsb);
...
class ClusterWrapper2 {
    ...
    // constructor param
    public ClusterWrapper2(String sourceIds, String summary, List<N2> recommendNs) {
